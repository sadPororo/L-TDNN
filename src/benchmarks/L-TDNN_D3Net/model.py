...
class SE_D2Block(nn.Module):
    def __init__(self, channels, depth, kernel_size=(3,3)):
        super().__init__()

        assert channels % depth == 0

        self.depth = depth
        self.channels = np.cumsum([channels // depth] * depth)[::-1].tolist()
        self.layers = nn.ModuleList([
            ConvReluBn(self.channels[d], self.channels[d], kernel_size, 2**d) for d in range(depth)
        ])

        self.se = SE_Module(channels, p_dropout=0)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < self.depth - 1:
                ...
                out_channels = x.size(1) - in_channels
                x, out = torch.split(x, [in_channels, out_channels], dim=1)
                output += (out,)

            else:
                output += (x,)
        
        output = self.se(torch.cat(output, dim=1))
        
        return output


class D3Block(nn.Module):
    def __init__(self, in_channels, channels, n_block, depth):
        super().__init__()

        self.in_conv = nn.Sequential(
            nn.BatchNorm2d(in_channels),
            ConvReluBn(in_channels, channels, kernel_size=(1,5))
        )
        self.se_d2blocks = nn.ModuleList([
            SE_D2Block(channels, depth) for _ in range(n_block)
        ])

        self.dense_channels = n_block * channels
    ...
    def forward(self, x):
        ...
        output = ()
        for i, layer in enumerate(self.se_d2blocks):
            x = layer(x)
            output += (x,)

        output = self.fusion(torch.cat(output, dim=1))
